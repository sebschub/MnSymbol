...
our $VERSION = "2.2";   # mf2pt1 version number
...
my $uniqueID;
my $designsize;
my ($mffile, $pt1file, $pfbfile, $pefile);
my $encoding;
my $rounding;
...
sub get_bboxes ($)
{
    execute_command 1, ("mpost", "-mem=mf2pt1", "-progname=mpost",
                        "\\mode:=localfont; mag:=$mag; input $mffile");
    ...
    foreach my $psfile (@charfiles) {
          ...
          MF2PT1_CMD:
            {
                ...
                $tokens[2] eq "font_underline_position" && do {
                    # We calculate $underlinepos later, after
                    # $designsize is defined.
                    my $tok3 = $tokens[3];
                    $underlinepos = sub {round ($tok3*1000/$designsize)};
                    last MF2PT1_CMD;
                };

                # font_underline_thickness number -- thickness of underline
                $tokens[2] eq "font_underline_thickness" && do {
                    # We calculate $underlinethick later, after
                    # $designsize is defined.
                    my $tok3 = $tokens[3];
                    $underlinethick = sub {round ($tok3*1000/$designsize)};
                    last MF2PT1_CMD;
                };
        ...
        close PSFILE;
        if (!$havebbox) {
            warn "${progname}: No beginchar in character $charnum; bbox is probably incorrect\n";
        }
    }
}
...
assign_default $creationdate, localtime;
...
assign_default $underlinepos, $opthash{underpos}, -100;
$underlinepos = $underlinepos->() if ref $underlinepos eq "CODE";
assign_default $underlinethick, $opthash{underthick}, 50;
$underlinethick = $underlinethick->() if ref $underlinethick eq "CODE";
...
Font encoding, either the name of a file which contains 256
whitespace-separated character names (e.g., "C<_a230>" or
"C<parenleft>") or one of C<standard> (the default), C<ot1>, or
C<isolatin1>
...
Fraction of a font unit (0.0 < I<number> <= 1.0) to which to round
coordinate values [default: 1.0]

=item B<--ffscript>=I<file.pe>

Script to pass to FontForge
